...
void GraphUtils::Set(const cpuGraph &g)
{
  std::fill(eidx_, eidx_ + g.num_v * g.num_v, UINT8_MAX);
  uint8_t edge_pos = 0u;

  for (uint32_t u = 0u; u < g.num_v; u++)
  {
    for (uint32_t offset = g.offsets_[u]; offset < g.offsets_[u + 1];
         offset++)
    {
      const uint32_t u_other = g.neighbors_[offset];

      uint32_t key = u * (g.num_v) + u_other;
      eidx_[key] = edge_pos;
      edge_pos++;
    }
  }
  ...
  {
    nbrbits_[i] = 0u;
    uint8_t *based = eidx_ + i * g.num_v;
    for (uint32_t j = 0; j < g.num_v; j++)
    {
